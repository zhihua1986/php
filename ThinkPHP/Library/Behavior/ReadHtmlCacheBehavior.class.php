...
class ReadHtmlCacheBehavior
{
   ...
   //        }
            ...
            if (false !== $cacheTime && $this->checkHTMLCache($Url, $cacheTime)) {
                //静态页面有效
                // 读取静态页面输出
    ...
    private static function requireHtmlCache()
    {
        ...
        if (!empty($htmls) && !$trackid && $islogin!=1 &&  !$query['t'] ){
...
// 某个控制器的静态规则
   ...
   //        }
                ...
                // 当前缓存文件
                define('HTML_FILE_NAME', HTML_PATH . $Url . C('HTML_FILE_SUFFIX', null, '.html'));
                return $cacheTime;
            }
        }
        // 无需缓存
