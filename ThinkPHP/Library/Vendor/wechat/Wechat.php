...
class Wechat
{
	...
	const API_BASE_URL_PREFIX = 'https://api.weixin.qq.com'; //sh.api.weixin.qq.com以下API接口URL需要使用此前缀
	...
	public function valid($return=false)
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] == "POST") {
        	...
        	$echoStr = $_GET["echostr"];
        	 ob_clean();//解决微信服务器验证token验证失败的问题
        	if ($return) {
    ...
    protected function log($log){
    		//if ($this->debug && function_exists($this->logcallback)) {
				...
				file_put_contents('./data/wechatlog.txt',$log."\n",FILE_APPEND);
