				...
				takungaeImgup : function(opt, serverCallBack) {
					...
					$(this).change(function() {
						...
						if (fileList.length > imageNum|| totalNum > imageNum) {
							alert("上传图片数目不可以超过9个，请重新选择"); // 一次选择上传超过5个
							...
							for ( var i = 0; i < fileList.length; i++) {
								...
								imgArr.push(imgUrl);
								var $section = $("<section class='up-section am-fl loading'>");
								imgContainer.children(".z_file").before($section);
								var $span = $("<span class='up-span'>");
								$span.appendTo($section);
								var $img0 = $("<i class='close-upimg' id='confirmBtn'></i>").on("click",function(event){
											    event.preventDefault();
												event.stopPropagation();
												$(".works-mask").show();
												delParent = $(this).parent();
								});
								$img0.addClass("am-icon-close").appendTo($section);
					...
					$(".wsdel-ok").click(function(event) {
						...
						delParent.remove();
						if($("#textarea").val()=="" && $(".up-section").length == 0 ){
							$("#confirm").attr("disabled","disabled").removeClass("btn-main");
						}
					});
