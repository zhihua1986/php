...
use Think\Model;
class itemscateModel extends Model
{
	
 protected $fields = array('id','name','ali_id','tags','pid','spid','remark','sort','wait_time','end_time','shop_type','mix_price'
 ,'max_price','mix_volume','max_volume','ems','thiscid','add_time','ordid','status','seo_title','seo_keys','seo_desc','cateimg');
protected $pk     = 'id';
    ...
    public function cate_cache() {
		$cate_list = array();
			...
			if(false === $cate_list = S('cate_list')){
				$cate_data = $this->field('id,pid,name,cateimg,ali_id,remark')->where('status=1')->order('ordid desc')->select();
