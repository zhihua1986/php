...
class BackupAction extends BaseAction
{
    ...
    public function index()
    {
        if (IS_POST || isset($_GET['dosubmit']))
        {
            ...
            } else
            {
                ...
                if (is_dir(TQK_DATA_PATH . $this->backup_path . $this->backup_name))
                {
                    $this->error(L('backup_name') . L('exists'));
                }

               
                mkdir(TQK_DATA_PATH . $this->backup_path . $this->backup_name);

                if (!is_file(TQK_DATA_PATH . $this->backup_path . $this->backup_name .
                    '/tbl_queue.log'))
                {
    ...
    public function import()
    {
        ...
        if ($this->_import_vol($backup['file']))
        {
            if ($vol < count($backups))
            {
                ...
                $vol++;
                $link = '/index.php?m=admin&c=backup&a=import&vol=' . $vol .
    ...
    {
        $sql_file = TQK_DATA_PATH . $this->backup_path . $this->backup_name . '/' . $sql_file_name;
...
if(strstr($_GET['backup'],'tqk_')){
        $dir=new Dir();
        $dir->delDir(TQK_DATA_PATH . $this->backup_path . $_GET['backup'].'/');             
    ...
    public function download()
    {
        ...
        $sql_file = TQK_DATA_PATH . $this->backup_path . $backup_name . '/' . $file;
        $allowExt = ['jpeg', 'jpg', 'png', 'gif','sql'];
    ...
    private function _get_vols($backup_name)
    {
        $vols = array(); //所有的卷
        $bytes = 0;
        $vol_path = TQK_DATA_PATH . $this->backup_path . $backup_name . '/';
        if (is_dir($vol_path))
        {
    ...
    {
        $backups = array(); //所有的备份
        if (is_dir(TQK_DATA_PATH . $this->backup_path))
        {
            if ($handle = opendir(TQK_DATA_PATH . $this->backup_path))
            {
                while (($file = readdir($handle)) !== false)
                {
                    if ($file{0} != '.' && filetype(TQK_DATA_PATH . $this->backup_path . $file) == 'dir')
                    {
                        $backup['name'] = $file;
                        $backup['date'] = filemtime(TQK_DATA_PATH . $this->backup_path . $file) - date('Z');
                        ...
                        $backup['total_size'] =round($this->_get_dir_size(TQK_DATA_PATH . $this->backup_path . $file)/1024,2);
                        $backups[] = $backup;
                    }
                }
            }
    ...
    private function _deal_result($tables, $vol, $sizelimit)
    {
        ...
        if (empty($tables))
        {
            ...
            $this->success(L('bakup_succ'), '/index.php?m=admin&c=backup&a=restore');
        } else
        {
            //开始下一卷
            $this->_set_vol($vol); //设置分卷记录
            $link = '/index.php?m=admin&c=backup&a=index&dosubmit=1&type=url&backup_name=' . $this->backup_name . '&sizelimit=' .
                $sizelimit;
    ...
    {
        $backup_path = TQK_DATA_PATH . 'backup/';
		$today	='tqk_'.rand(10000,99999).'_';
    ...
    {
        return file_put_contents(TQK_DATA_PATH . $this->backup_path . $this->backup_name .
    ...
    {
        $tbl_queue_file = TQK_DATA_PATH . $this->backup_path . $this->backup_name .
            '/tbl_queue.log';
    ...
    {
        $log_file = TQK_DATA_PATH . $this->backup_path . $this->backup_name . '/vol.log';
