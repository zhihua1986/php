...
class ThirdorderAction extends BaseAction
{
    ...
    public function sync_order()
    {
        ...
        $type = I('type');
        $result_data = $this->sync_scene($page, $starttime, $pagesize,$type);
     ...
     * @return array
     */
    protected function sync_scene($page, $starttime, $pagesize,$type)
    {
        if (function_exists('opcache_invalidate')) {
            $basedir = $_SERVER['DOCUMENT_ROOT'];
            $dir = $basedir . '/data/Runtime/Data/dmjiesuan.php';
   ...
   (select nickname from ' . $prefix . 'user where ' . $prefix . 'user.id = ' . $prefix . 'dmorder.uid limit 1) as nickname';
        ...
        foreach ($rows as $k => $v) {
            ...
            $list[$k]['id'] = $v['id'];
            $list[$k]['smstitle'] = '<font color="red">[' . $this->OrderSrc($v['src']) . ']</font>' . $v['goods_name'];
