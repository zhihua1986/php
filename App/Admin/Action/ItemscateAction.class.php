...
class ItemscateAction extends BaseAction{
    ...
    public function index(){
        ...
        foreach($result as $r){
            ...
            $r['str_manage'] = '<a href="javascript:;" class="J_showdialog" data-uri="' . U('itemscate/add', array('pid' => $r['id'])) . '" data-title="' . L('add_item_cate') . '" data-id="add" data-width="520" data-height="20">' . L('add_item_subcate') . '</a> |
                                <a href="javascript:;" class="J_showdialog" data-uri="' . U('itemscate/edit', array('id' => $r['id'])) . '" data-title="' . L('edit') . ' - ' . $r['name'] . '" data-id="edit" data-width="500" data-height="20">' . L('edit') . '</a> |
                                <a href="javascript:;" class="J_confirmurl" data-acttype="ajax" data-uri="' . U('itemscate/delete', array('id' => $r['id'])) . '" data-msg="' . sprintf(L('confirm_delete_one'), $r['name']) . '">' . L('delete') . '</a>';
        ...
        $big_menu = array('title' => L('add_item_cate'), 'iframe' => U('itemscate/add'), 'id' => 'add', 'width' => '520', 'height' => '80');
    ...
    public function ajax_getchilds(){
        ...
        $id = I('id', '0','intval');
        $map = array('pid' => $id);
        $return = $this -> _mod -> field('id,name,ali_id') -> where($map) -> select();
        if ($return){
